...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Daftar Peminjaman</h1>
            <p class="text-gray-500">Kelola peminjaman buku anggota</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-blue-100">
        <div class="bg-blue-50 px-6 py-4 border-b border-blue-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <form method="GET" action="{{ route('petugas.peminjaman.index') }}" class="w-full sm:w-auto">
                <div class="relative">
                        ...
                        <td class="px-6 py-4 align-top text-center">
                            <div class="flex flex-col gap-2">
                                ...
                                @if($item->status == 'pending')
                                    ...
                                    <form action="{{ route('petugas.peminjaman.reject', $item->id) }}" method="POST" onsubmit="return confirm('Tolak peminjaman ini?')">
                                        @csrf @method('PATCH')
                                ...
                                @elseif($item->status == 'dipinjam')
                                    <form action="{{ route('petugas.peminjaman.update', $item->id) }}" method="POST" onsubmit="return confirm('Buku dikembalikan?')">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="status" value="dikembalikan">
                                        <button class="w-full px-4 py-2 bg-gray-800 text-white rounded-lg text-xs font-bold hover:bg-gray-900 transition shadow-sm flex items-center justify-center gap-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg> Dikembalikan
                                        </button>
                                    </form>
                                @endif
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-yellow-50/50 bg-white transition">
                        ...
                        <td class="px-6 py-4 align-top text-center">
                            <div class="flex flex-col gap-2">
                                <form action="{{ route('petugas.peminjaman.approve-kembali', $item->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <button class="w-full px-4 py-2 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700 transition shadow-sm flex items-center justify-center gap-1">
                                ...
                                <form action="{{ route('petugas.peminjaman.reject-kembali', $item->id) }}" method="POST" onsubmit="return confirm('Tolak pengembalian ini?')">
                                    @csrf @method('PATCH')
                                    <button class="w-full px-4 py-2 bg-red-100 text-red-600 rounded-lg text-xs font-bold hover:bg-red-200 transition shadow-sm flex items-center justify-center gap-1">
