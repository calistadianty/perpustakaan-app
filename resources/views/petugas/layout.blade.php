...
<head>
    ...
    <title>Petugas Panel - {{ config('app.name', 'Rumah Baca') }}</title>
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 text-gray-600 transform transition-transform duration-300 lg:translate-x-0 -translate-x-full overflow-y-auto max-h-screen">
            ...
            <div class="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">Petugas Panel</p>
                </div>
            </div>
            ...
            <nav class="mt-6 px-4">
                <div class="space-y-2">
                    ...
                    <a href="{{ route('petugas.dashboard') }}" 
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium {{ request()->routeIs('petugas.dashboard') ? 'active' : '' }}">
                    ...
                    <!-- Manajemen Buku -->
                    <div x-data="{ 
                        open: {{ request()->routeIs('petugas.books.*') || request()->routeIs('petugas.categories.*') ? 'true' : 'false' }} 
                    }">

                        <button @click="open = !open" 
                            class="sidebar-link w-full flex items-center justify-between gap-3 px-4 py-3 rounded-lg text-sm font-medium
                            {{ request()->routeIs('petugas.books.*') || request()->routeIs('petugas.categories.*') ? 'active' : '' }}">
                        ...
                        <div x-show="open" x-collapse class="pl-12 mt-1 space-y-1">
                            <a href="{{ route('petugas.books.index') }}"
                               ...
                               {{ request()->routeIs('petugas.books.*') ? 'text-blue-600 font-bold' : 'text-gray-600' }}">
                                Daftar Buku
                            </a>

                            <a href="{{ route('petugas.categories.index') }}"
                               class="block px-4 py-2 text-sm rounded-lg hover:bg-gray-50
                               {{ request()->routeIs('petugas.categories.*') ? 'text-blue-600 font-bold' : 'text-gray-600' }}">
                                Kategori
                            </a>
                        </div>
                    </div>
                    ...
                    <!-- Peminjaman -->
                    <div x-data="{ 
                        open: {{ request()->routeIs('petugas.peminjaman.*') ? 'true' : 'false' }} 
                    }">

                        <button @click="open = !open" 
                            class="sidebar-link w-full flex items-center justify-between gap-3 px-4 py-3 rounded-lg text-sm font-medium
                            {{ request()->routeIs('petugas.peminjaman.*') ? 'active' : '' }}">
                        ...
                        <div x-show="open" x-collapse class="pl-12 mt-1 space-y-1">
                            <a href="{{ route('petugas.peminjaman.index') }}"
                               class="block px-4 py-2 text-sm rounded-lg hover:bg-gray-50
                               {{ request()->routeIs('petugas.peminjaman.index') ? 'text-blue-600 font-bold' : 'text-gray-600' }}">
                                Sedang Dipinjam
                            </a>

                            <a href="{{ route('petugas.peminjaman.history') }}"
                               class="block px-4 py-2 text-sm rounded-lg hover:bg-gray-50
                               {{ request()->routeIs('petugas.peminjaman.history') ? 'text-blue-600 font-bold' : 'text-gray-600' }}">
                                Riwayat
                            </a>
                        </div>
                    </div>
                    ...
                    <a href="{{ route('petugas.laporan.index') }}" 
                       ...
                       {{ request()->routeIs('petugas.laporan.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('petugas.users.index') }}" 
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium
                       {{ request()->routeIs('petugas.users.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 bg-gray-50">
                <div class="flex items-center gap-3 px-2">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-blue-600">Staff / Petugas</p>
                    </div>
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-64">
            ...
            <footer class="bg-white border-t px-6 py-4">
                <p class="text-center text-sm text-gray-500">
                    © {{ date('Y') }} Rumah Baca. Petugas Panel.
                </p>
            </footer>
        </div>
    </div>
