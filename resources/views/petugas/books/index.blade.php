...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500">Kelola buku yang Anda ajukan</p>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                @if($book->cover)
                                    <img src="{{ Storage::url($book->cover) }}" alt="{{ $book->judul }}" class="w-12 h-16 object-cover rounded-lg border border-gray-100">
                                @else
                                    <div class="w-12 h-16 bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            @forelse($book->categories as $category)
                                <span class="inline-block bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-medium mb-1 border border-blue-100">
                                    {{ $category->nama }}
                                </span>
                            @empty
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-block {{ $book->stok > 0 ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }} px-3 py-1 rounded-full text-xs font-medium">
                                {{ $book->stok }} tersedia
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('petugas.books.show', $book) }}"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <a href="{{ route('petugas.books.create') }}" class="mt-4 text-blue-600 font-medium hover:underline">
                                    Tambah buku pertama →
                                </a>
                            </div>
                        </td>
