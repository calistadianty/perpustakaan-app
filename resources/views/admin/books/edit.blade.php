...
<div class="space-y-6">
    ...
    <!-- Form -->
     <div class="max-w-3x3">
    <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-8 space-y-6">
        @csrf
        @method('PUT')
                    ...
                    @foreach($categories as $category)
                        <label class="flex items-center space-x-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition">
                            <input type="checkbox" name="category_id[]" value="{{ $category->id }}" 
                                {{ in_array($category->id, old('category_id', $book->categories->pluck('id')->toArray())) ? 'checked' : '' }}
            ...
            <div>
                ...
                @if($book->cover)
                    <div class="mb-3 flex items-center gap-4">
                        <img src="{{ Storage::url($book->cover) }}" alt="Current cover" class="w-16 h-20 object-cover rounded-lg">
